 ...
 *
 * WHAT IT DOES
 *   - Forwards IPv4 by exact-match ingress_port (tb_hop_fwd) — see TOPOLOGY
 *     below for why this replaced dst-IP forwarding.
 ...
 *
 *
 * TOPOLOGY (3-switch loopback chain on a single Tofino, one P4 program)
 *   Xena Port 0 -> 7/0  (hop 1 in) -> [fwd] -> 17/0 (hop 1 out)
 *                                                  |  DAC loopback cable
 *                                              18/0 (hop 2 in) -> [fwd] -> 19/0 (hop 2 out)
 *                                                                              |  DAC loopback cable
 *                                                                          20/0 (hop 3 in) -> [fwd] -> 8/0 -> Xena Port 1
 *   The packet re-enters the SAME ingress pipeline on a different physical
 *   port at each hop, so hdr.ipv4.dst_addr is identical on all 3 passes —
 *   forwarding and switch identity are therefore keyed on ingress_port, not
 *   on dst IP. INT-XD reports fire once per hop (3 total per packet), all
 *   exiting via mirror session 2 / dev_port 60 (9/0) to the collector, same
 *   as the single-hop case.
 *
 */
    ...
    inout ingress_intrinsic_metadata_for_tm_t       ig_tm_md)
{
    ...
    // Hop-based forwarding for the 3-switch loopback chain: the next-hop
    // egress port is fixed by physical cabling, not by dst IP, because the
    // same flow re-enters this same ingress pipeline on a different
    // front-panel port at each hop (dst_addr is identical on all 3 passes).
    table tb_hop_fwd {
    ...
    inout egress_intrinsic_metadata_for_output_port_t eg_oport_md)
{
    ...
    // Keyed on which physical port the original packet ingressed on for
    // THIS pass (carried via mirror_md.ingress_port, set in set_xd_clone()).
    // This is what gives each of the 3 chained hops its own switch_id /
    // dp_mac / dp_ip in the report, even though it's the same flow and the
    // same physical ASIC pipeline executing each time.
    table tb_int_xd_reporting {
        key = { hdr.mirror_md.ingress_port : exact; }
